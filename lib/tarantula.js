	...
	function() { return frontier.length },
		...
		if (!err && response.statusCode == 200) {
			...
			$('a').each(function() {
				...
				var resolved = url.resolve(rq.uri, l);
				if (brain.shouldVisit(resolved))
				    if (!brain.isSeen(resolved))
					if (frontier.indexOf(resolved) == -1 && nb.indexOf(resolved) == -1) {
					    debug('Add to neighbourhood: ' + resolved);
					    nb.push(resolved);
					}
			    }
			});
