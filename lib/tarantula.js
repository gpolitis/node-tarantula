...
function Tarantula (brain) {
	...
	this.tasks = new PriorityQueue(this.compareTasks);
...
Tarantula.prototype.push = function (uris, pageUri, parentTask) {
	...
	if (uris.length) {
		var depth = parentTask ? parentTask.depth + 1 : 0;
		...
		tasks.forEach(this.tasks.enq.bind(this.tasks));
		setImmediate(this.churn);
	}
	return uris;
};
/**
...
Tarantula.prototype.legEvent = function (leg, task, eventName) {
	...
	if (arguments[4] instanceof Error && arguments[4].syscall === 'spawn') {
		console.error('PhantomJS does not appear to be installed.');
	}
};
/**
...
/**
 * Follow more tasks as long as we legs available
 */
Tarantula.prototype.churn = function () {
	if (this.tasks.isEmpty() && !this.legs.active) {
		this.legs.drain();
		this.complete();
	}
	else {
		while (!this.tasks.isEmpty() && this.legs.active < this.brain.legs) {
			var leg = this.legs.acquire();
			var task = this.tasks.deq();
			setImmediate(this.follow.bind(this, task, leg));
		}
	}
};
/**
 * Comparator for tasks
 * We want the smallest depth to go first
 *
 ...
 * @return {int}
 *   Relative difference in depth
 */
Tarantula.prototype.compareTasks = function (a, b) {
