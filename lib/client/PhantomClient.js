 ...
 * @param
 */
PhantomClient.prototype.setStrictSSL = function (strictSSL) {
	this.phantomSettings.parameters['ignore-ssl-errors'] = strictSSL ? 'no' : 'yes';
	return this;
};
/**
 * Sets HTTP Headers
...
PhantomClient.prototype._get = function () {
	...
	this.phantom.createPage(function(err, page) {
		if (err || !page) {
			return thisClient.emit('error', 'phantom.createPage', err, page);
		}
		...
		return page.open(thisClient.uri, function(err, status) {
			if (err || status !== "success") {
				return thisClient.emit('error', 'phantom.page.open', err, status);
			}
			...
			page.includeJs('//ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js', function (err) {
				if (err) {
					return thisClient.emit('error', 'phantom.page.includeJs', err);
				}
